...
object ExecutionStoreBenchmark extends Bench[Double] with DefaultJsonProtocol {
  ...
  val graph = namespace.toWomExecutable(inputJson, NoIoFunctionSet, checkForUnwantedInputs = true).getOrElse(throw new Exception("Failed to build womExecutable")).graph
