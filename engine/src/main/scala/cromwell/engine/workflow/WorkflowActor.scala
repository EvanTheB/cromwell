...
import cromwell.engine.workflow.workflowstore.WorkflowStoreActor.WriteWorkflowHeartbeatCommand
  ...
  def props(workflowId: WorkflowId,
    ...
    Props(
      new WorkflowActor(
        ...
        heartbeatTtl = heartbeatTtl)).withDispatcher(EngineDispatcher)
  }
}
...
class WorkflowActor(val workflowId: WorkflowId,
  ...
  // Send heartbeats a bit redundantly but unacked.
  timers.startPeriodicTimer(workflowId, SendWorkflowHeartbeatCommand, heartbeatTtl / 3)
  ...
  private def sendHeartbeat(): Unit = workflowStoreActor ! WriteWorkflowHeartbeatCommand(workflowId)
