...
case class Workflow private(
  ...
  def womExecutable(validator: RequirementsValidator, inputFile: Option[String] = None, ioFunctions: IoFunctionSet, checkForUnwantedInputs: Boolean): Checked[Executable] = {
    CwlExecutableValidation.buildWomExecutableCallable(womDefinition(validator, Vector.empty), inputFile, ioFunctions, checkForUnwantedInputs)
  }
