...
package object cwl extends TypeAliases {
  ...
  implicit class CwlHelper(val cwl: Cwl) extends AnyVal {
    def womExecutable(validator: RequirementsValidator, inputsFile: Option[String], ioFunctions: IoFunctionSet, checkForUnwantedInputs: Boolean): Checked[Executable] = {
      def executable = cwl match {
        case Cwl.Workflow(w) => w.womExecutable(validator, inputsFile, ioFunctions, checkForUnwantedInputs)
        case Cwl.CommandLineTool(clt) => clt.womExecutable(validator, inputsFile, ioFunctions, checkForUnwantedInputs)
        case Cwl.ExpressionTool(et) => et.womExecutable(validator, inputsFile, ioFunctions, checkForUnwantedInputs)
      }
